...
export default function SubmissionsPage() {
  ...
  const load = async () => { setLoading(true); setError(''); try { const [r, c, l] = await Promise.all([api.get('/submissions'), api.get('/courses'), api.get('/lessons')]); setItems(r.items || []); setCourses(c.items || []); setLessons(l.items || []); } catch (e) { setError(e.message); } finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
  useEffect(() => {
    if (!form.courseId && courses[0]) setForm((prev) => ({ ...prev, courseId: courses[0].id }));
  }, [courses, form.courseId]);
  useEffect(() => {
    const filtered = lessons.filter((l) => l.courseId === form.courseId);
    if (!form.lessonId && filtered[0]) setForm((prev) => ({ ...prev, lessonId: filtered[0].id }));
  }, [lessons, form.courseId, form.lessonId]);
