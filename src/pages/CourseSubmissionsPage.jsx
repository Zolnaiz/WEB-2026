...
export default function CourseSubmissionsPage() {
  ...
  const { items, loading, error, page, totalPages, setPage, refetch } = useSubmissions({ course_id, status, search });

  const applyFilters = () => {
    refetch({ course_id, status, search, page: 1 });
    setPage(1);
  };
    ...
    <section className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <input
            className="rounded-md border border-slate-300 p-2.5 text-sm"
